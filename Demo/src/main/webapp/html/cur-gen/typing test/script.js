...
function startTest() {
    if (!isTestActive) {
        ...
        userInput.focus();
        startButton.textContent = 'བརྟག་ཚོད་མཚམས་འཇོག་';
...
function displayResults(time, speed, errorRateValue, errors) {
    ...
    errorList.innerHTML = '';
    const errorChars = {};
    for (let i = 0; i < Math.min(currentText.length, userInput.value.length); i++) {
        if (currentText[i] !== userInput.value[i]) {
            const char = userInput.value[i] || 'མེད་པ།';
